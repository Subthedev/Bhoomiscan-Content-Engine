...
// Week A and B are the base patterns (strategy Section 4.1)
// Week C and D are computed by shifting: C = shift A by +3 slots, D = shift B by +3 slots
// This ensures variety while maintaining the archetype distribution rules
const WEEK_A = [1,2,11,3,4,12,5,6,7,8,9,1,10,2,6,11,3,4,7,5];
const WEEK_B = [4,6,10,7,9,11,1,12,8,2,5,3,6,4,1,9,7,12,11,10];
function shiftHooks(base, offset) {
  const r = [];
  for (let i = 0; i < base.length; i++) {
    // Shift the archetype ID cyclically through 1-12
    r.push(((base[i] - 1 + offset) % 12) + 1);
  }
  return r;
}
const WEEK_HOOKS = [
  WEEK_A,
  WEEK_B,
  shiftHooks(WEEK_A, 3),  // Week C = A shifted by +3
  shiftHooks(WEEK_B, 3),  // Week D = B shifted by +3
];
...
[ ] Hook scrollstopper in 1.5s? [ ] 130-150 words? [ ] Chai tapri feel?
...
function scoreFinding(text, pain, season, angles) {
  ...
  // +3 if matches current pain cycle
  if (pain && lower.includes(pain.p)) s += 3;
  if (pain && lower.includes(pain.s)) s += 2;
  ...
  if (angles?.some(a => lower.includes(a.toLowerCase().split(" ")[0]))) s += 2;
  return s;
}

// Classify a chunk by audience — returns sorted array of matching audiences
function classifyAudience(text) {
  const lower = text.toLowerCase();
  const scores = {};
  for (const [aud, kws] of Object.entries(RES_KW)) {
    scores[aud] = kws.reduce((s, kw) => s + (lower.includes(kw) ? 1 : 0), 0);
  }
...
// Classify a chunk by content pillar
function classifyPillar(text) {
  const lower = text.toLowerCase();
  const scores = {};
  for (const [pil, kws] of Object.entries(PIL_KW)) {
    scores[pil] = kws.reduce((s, kw) => s + (lower.includes(kw) ? 1 : 0), 0);
  }
  const max = Math.max(...Object.values(scores));
...
function processResearch(rawText, context, rotation) {
  // Split into meaningful chunks (double newline, bullet, numbered item)
  ...
  const findings = chunks.map(chunk => {
    ...
    return { text: chunk.slice(0, 300), audiences, pillar, stats, score };
  });
  ...
  function fmtBatch(label, items, allItems) {
    ...
    if (statItems.length) {
      out += `KEY STATS:\n${statItems.slice(0, 3).map(f => `  • ${f.text.slice(0, 150)} [${f.pillar}]`).join("\n")}\n`;
    }
    if (trendItems.length) {
      out += `TRENDS:\n${trendItems.slice(0, 2).map(f => `  • ${f.text.slice(0, 150)} [${f.pillar}]`).join("\n")}\n`;
    }
    if (storyItems.length) {
      out += `STORY HOOKS:\n${storyItems.slice(0, 2).map(f => `  • ${f.text.slice(0, 150)}`).join("\n")}\n`;
    }
    if (eduItems.length) {
      out += `INSIGHTS:\n${eduItems.slice(0, 2).map(f => `  • ${f.text.slice(0, 150)} [${f.pillar}]`).join("\n")}\n`;
    }
    return out;
  }
...
// 3 Platform-Specific Research Prompts (dynamic date injected at copy time)
  ...
  return [
    {
      ...
      prompt: `TODAY IS: ${dateStr} (${isoDate}). You are researching for BhoomiScan — India's land verification and listing platform focused on Odisha.
...
MINIMUM 10 findings. Prioritize findings with HARD NUMBERS and STATS — these make the strongest reel hooks.`
    },
    {
      ...
      prompt: `Current date: ${dateStr} (${isoDate}). Use your real-time search and Google integration to find the FRESHEST data.

TASK: Research Indian real estate and land market developments from the last 7 days for BhoomiScan (land verification + listing platform, Odisha-focused).
...
MINIMUM 10 findings. Latest data only — reject anything older than 7 days from ${isoDate}.`
    },
    {
      ...
      prompt: `Date: ${dateStr} (${isoDate}). Research with FULL CITATIONS for BhoomiScan content team.
...
ALWAYS: "tum/tumhare" | "link bio mein hai" (rotate) | 130-150 words | 6-beat energy arc`;
...
const HOOK_COMPAT = {
  ...
  motivation:{best:[9,11,12],  avoid:[7,8]},
  myth:     {best:[7,1,11],    avoid:[6,5]},
  community:{best:[8,10,7],    avoid:[3,9]},
  general:  {best:[],          avoid:[]},
};

// Given assigned hooks and angles for a batch, swap any incompatible hooks
function enforceHookCompat(hooks, angles, seed) {
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    if (compat.avoid.includes(result[i])) {
      // Try to swap with another position in the batch that doesn't conflict
      let swapped = false;
      for (let j = 0; j < result.length; j++) {
        if (j === i) continue;
        const otherType = classifyAngle(angles[j]);
        const otherCompat = HOOK_COMPAT[otherType];
        // Check: result[j] is OK for angle[i], and result[i] is OK for angle[j]
      ...
      if (!swapped && compat.best.length) {
        // Last resort: pick a best hook not already used in this batch
        const used = new Set(result);
        for (const bh of compat.best) {
          if (!used.has(bh)) { result[i] = bh; break; }
        }
      }
    }
  }
...
function pickLru(total,hist,count,seed){
  const sc={};for(let i=0;i<total;i++)sc[i]=0;
  hist.forEach((used,wi)=>{const rec=hist.length-wi;(used||[]).forEach(idx=>{if(sc[idx]!==undefined)sc[idx]=Math.max(sc[idx],rec);});});
  const items=shuf(Object.keys(sc).map(Number),seed||7);
  items.sort((a,b)=>sc[a]-sc[b]);
  return items.slice(0,Math.min(count,total));
}

// Allocate CTAs across 4 batches of 5, respecting weekly totals (4,3,3,4,3,3)
// and ensuring each batch has 5 unique CTA patterns
function allocCtas(seed){
  ...
  // Distribute to 4 batches, ensuring uniqueness within each batch
  const batches=[[],[],[],[]];
  const used=[new Set(),new Set(),new Set(),new Set()];
  // First pass: assign greedily to batches that need items and don't have this CTA yet
  for(const cta of shuffled){
    let placed=false;
    for(let b=0;b<4;b++){
      if(batches[b].length<5&&!used[b].has(cta)){batches[b].push(cta);used[b].add(cta);placed=true;break;}
    }
    if(!placed){
      // Find any batch with space (will create duplicate — swap later)
      for(let b=0;b<4;b++){if(batches[b].length<5){batches[b].push(cta);placed=true;break;}}
    }
  }
  // Second pass: fix any within-batch duplicates by swapping with other batches
  for(let b=0;b<4;b++){
    const seen=new Set();
    for(let i=0;i<batches[b].length;i++){
      if(seen.has(batches[b][i])){
        // Find another batch with a CTA we don't have, that doesn't have ours
        for(let ob=0;ob<4;ob++){
          ...
          for(let j=0;j<batches[ob].length;j++){
            ...
            if(!seen.has(theirs)&&!new Set(batches[ob]).has(ours)){
              // Check theirs won't create dup in ob
              const obSet=new Set(batches[ob]);obSet.delete(theirs);
              if(!obSet.has(ours)){batches[ob][j]=ours;batches[b][i]=theirs;break;}
            }
          }
  ...
  return {b1:batches[0],b2:batches[1],b3:batches[2],b4:batches[3]};
}

function calc(wk,mo,history){
  const ci=(wk-1)%4,se=CAL.find(c=>c.m===mo)||CAL[0];
  // Pick content angles FIRST (needed for hook compatibility check)
  const ba=pickLru(ANG.buyer.length,(history||[]).map(w=>w?.ang?.buyer||[]),5,wk*101);
  const sa=pickLru(ANG.seller.length,(history||[]).map(w=>w?.ang?.seller||[]),5,wk*103);
  const aa=pickLru(ANG.agent.length,(history||[]).map(w=>w?.ang?.agent||[]),3,wk*107);
  const na=pickLru(ANG.nri.length,(history||[]).map(w=>w?.ang?.nri||[]),2,wk*109);
  ...
  // Cross-week audience-hook dedup: check if same hook was used for same audience last week
  ...
  // Enforce topic-hook compatibility (FIX 4)
  ["b1","b2","b3","b4"].forEach(bk=>{
    hB[bk]=enforceHookCompat(hB[bk],angStr[bk],wk);
  });
  const cB=allocCtas(wk*173);
  ...
  const bio={b1:shuf([0,1,2,3,4],wk*311),b2:shuf([0,1,2,3,4],wk*317),b3:shuf([0,1,2,3,4],wk*331),b4:shuf([0,1,2,3,4],wk*337)};
  return {
...
3. 130-150 words — COUNT CAREFULLY
...
function mkP4(cx,r,res){
  const se=r.se;
  const hasResearch = !!res;
  const freshness = hasResearch
    ? `FRESHNESS: S1 MUST use this week's research as shock hook | S2 MUST be forward-looking backed by research trends | S4 MUST ref festivals | S5 MUST drive comments/community`
...
function mkClean(){
  return `═══ MASTER CLEANUP ═══\n"Review Script [N] from [Buyer/Seller/Agent+NRI/Trending]. Issue: [describe].\nMaintain archetype/CTA/USP/130-150 words.\n\nFIXES:\n1. Hook weak → stronger SAME archetype\n2. CTA forced → different pattern\n3. Tone off → 2-3 texture elements\n4. Flat → booster at 40-60%\n5. AI → chai tapri speech\n6. Too long/short → trim/expand\n7. USP forced → natural weave\n8. Hinglish → Buyer 65%, Seller 70%, Agent 55%, NRI 60%\n9. Format wrong → match assigned\n10. bhai count → 2-4x only\n11. Pronouns → tum/tumhare only\n\nOutput corrected script + caption + hashtags. Highlight changes.\nRe-verify against Quality Gate."`;
}

// ═══ SMART LOG PARSER — handles ALL paste formats ═══
// Known values for context-validation (proves an S[1-5] match is actually a log row)
const LOG_TRIGGERS=["SAVE","SHARE","COMMENT","FOLLOW"];
const LOG_ENERGY=["HIGH","MEDIUM","WARM","LOW"];
const LOG_FORMATS=["Talking Head","Myth Buster","Story-Based","Story Based","Number Shock","Rapid Fire","Q&A Response","QA Response","Prediction","Future"];
const LOG_HOOKS=["Confession","Knowledge Gap","Single Number","Story Entry","Situational","Scene Setter","Common Mistake","Audience Question","Future Frame","Social Proof","Bold Claim","Rhetorical Challenge","Rhetorical"];
const LOG_CTAS=["Casual Aside","Logical Extension","Reassurance","Reassurance Bridge","Conditional","Process Integr","Process Integration","Platform-as-Solution","Platform"];
const LOG_USPS=["Verified Props","Verified","Free Listings","Free","WhatsApp Connect","WhatsApp","Zero Commission","Zero"];
const LOG_PAINS=["Trust Deficit","Trust","Digital Illiteracy","Digital","Commission Drain","Commission","Document Chaos","Document","Spam"];
// Word-boundary version for structured text (lines with delimiters)
const LOG_CTX_RX=new RegExp("\\b("+[...LOG_TRIGGERS,...LOG_ENERGY,...LOG_HOOKS,...LOG_CTAS].join("|")+")\\b","i");
// No-boundary version for raw concatenated text (Claude web UI strips formatting)
const LOG_CTX_RAW=new RegExp("("+[...LOG_TRIGGERS,...LOG_ENERGY,"Talking Head","Myth Buster","Number Shock","Story.Based","Rapid Fire",...LOG_HOOKS,...LOG_CTAS].join("|")+")","i");

function parseLog(text){
  const empty={batches:[],scripts:0,batchCount:0,completeBatches:0,rows:[],valid:false,details:[]};
  if(!text||!text.trim())return empty;

  const rows=[];

  // === STRATEGY 1: Line-based detection (pipe-delimited OR tab-delimited OR space-delimited) ===
  const lines=text.split("\n");
  for(const line of lines){
    // Skip header/separator lines
    if(/^\s*\|?\s*-+/.test(line))continue;
    if(/^\s*\|?\s*Script\s*[\t|]/i.test(line))continue;
    if(/^\s*Script\s+(Hook|CTA)/i.test(line))continue;

    // A) Pipe-delimited: | S1 | ... | or |S1| ...
    const pipeM=line.match(/\|\s*(S[1-5])\s*\|/i);
    if(pipeM){rows.push({id:pipeM[1].toUpperCase(),raw:line});continue;}

    // B) Line starts with S[1-5] (tab/space/comma delimited)
    const startM=line.match(/^\s*(S[1-5])\b/i);
    if(startM&&LOG_CTX_RX.test(line)){rows.push({id:startM[1].toUpperCase(),raw:line});continue;}

    // C) Line contains S[1-5] after a pipe or tab but not at strict start
    const midM=line.match(/[\t|]\s*(S[1-5])\s*[\t|]/i);
    if(midM&&LOG_CTX_RX.test(line)){rows.push({id:midM[1].toUpperCase(),raw:line});continue;}
  }

  // === STRATEGY 2: Raw concatenated text (Claude web UI copies strip pipes/newlines) ===
  // Triggered when line-based parsing found fewer than 5 scripts
  if(rows.length<5){
    const raw=text.replace(/\n/g," ");
    const candidates=[];
    const sRx=/S([1-5])/gi;
    let m;
    while((m=sRx.exec(raw))!==null){
      const idx=m.index;
      const num=m[1];
      const after=raw.slice(idx,Math.min(raw.length,idx+300));
      // Must NOT be inside a longer number like S10, S15
      const charAfterNum=raw[idx+2]||"";
      if(/[0-9]/.test(charAfterNum))continue;
      // Use NO-BOUNDARY regex for concatenated text context check
      if(LOG_CTX_RAW.test(after)){
        candidates.push({id:"S"+num,idx,after});
      }
    }
    if(candidates.length>=5){
      rows.length=0;
      for(const c of candidates){
        rows.push({id:c.id,raw:c.after.slice(0,200)});
      }
    }
  }

  // === STRATEGY 3: Markdown-formatted tables with ** or other markup ===
  if(rows.length<5){
    for(const line of lines){
      const fmtM=line.match(/\*?\*?\s*(S[1-5])\s*\*?\*?\s*\|/i)||line.match(/`(S[1-5])`/i);
      if(fmtM&&LOG_CTX_RX.test(line)&&!rows.find(r=>r.raw===line)){
        rows.push({id:fmtM[1].toUpperCase(),raw:line});
      }
    }
  }
  ...
  // === GROUP INTO BATCHES: every consecutive set of S1→S5 = 1 batch ===
  const batches=[];
  let cur=[];
  for(const row of rows){
    const num=parseInt(row.id[1]);
    if(cur.length>0){
      const lastNum=parseInt(cur[cur.length-1].id[1]);
      if(num<=lastNum){batches.push([...cur]);cur=[];}
    }
    cur.push(row);
  }
  if(cur.length>0)batches.push(cur);

  // Build details per batch
  const details=batches.map((b,i)=>{
    const ids=b.map(r=>r.id);
    const missing=["S1","S2","S3","S4","S5"].filter(s=>!ids.includes(s));
    return{batch:i+1,count:b.length,scripts:ids,missing,complete:missing.length===0};
  });
  const completeBatches=details.filter(d=>d.complete).length;
  return{batches,scripts:rows.length,batchCount:batches.length,completeBatches,rows,valid:completeBatches>=4,details};
}

// ═══ STORAGE ═══
const SK="bhoomiscan-v3";
...
export default function App(){
  ...
  const onFiles=(e)=>{
    ...
    newFiles.forEach(f=>{
      ...
      if(['pdf','doc','docx'].includes(ext)){
        // For binary formats, just store the filename and note
        setFiles(p=>[...p,{name:f.name,text:`[${ext.toUpperCase()} file - ${(f.size/1024).toFixed(1)}KB] Paste extracted text manually below`,size:f.size,type:ext}]);
      }else{
        r.onload=(ev)=>setFiles(p=>[...p,{name:f.name,text:ev.target.result,size:f.size,type:ext}]);
        r.readAsText(f);
      }
    });
    e.target.value="";
    flash(`${newFiles.length} file(s) added`);
  };
  ...
  const generate=()=>{
    ...
    setPr([
      {t:"Prompt 1 — Buyer (5 Scripts)",tx:mkP1(context,rotation,processed?.b1||rawRes)},
      {t:"Prompt 2 — Seller (5 Scripts)",tx:mkP2(context,rotation,processed?.b2||rawRes)},
      {t:"Prompt 3 — Agent+NRI (3+2)",tx:mkP3(context,rotation,processed?.b3||rawRes)},
      {t:"Prompt 4 — Trending+Mixed (5)",tx:mkP4(context,rotation,processed?.b4||rawRes)},
      {t:"Cleanup — Master Template",tx:mkClean()},
    ]);
    ...
    const resMsg=processed?` — ${processed.counts.total} findings (${processed.counts.withStats} stats)`:"";
  ...
  const logWeek=()=>{
    ...
    const rec={dt:cx.dt,wk:cx.wk,mo:cx.mo,yr:cx.yr,se:cx.se,pat:rot.pat,
  ...
  const sB=(v)=>({padding:"10px 18px",borderRadius:8,cursor:"pointer",fontSize:12,fontWeight:700,border:"none",transition:"all 0.2s",letterSpacing:0.3,
  ...
  const sBg=(v)=>({display:"inline-block",padding:"3px 9px",borderRadius:6,fontSize:10,fontWeight:700,letterSpacing:0.3,marginRight:4,marginBottom:3,
  ...
  const now=new Date(),cW=wkNum(now),cM=now.getMonth()+1,cSe=CAL.find(c=>c.m===cM)||CAL[0];
    ...
    <div style={{minHeight:"100vh",background:BG,color:TX,fontFamily:"'Segoe UI',Helvetica,Arial,sans-serif"}}>
      ...
      <div style={{background:`linear-gradient(135deg,${G},${GL})`,padding:"14px 20px",boxShadow:"0 2px 12px rgba(45,106,79,0.15)"}}>
        <div style={{maxWidth:960,margin:"0 auto",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div><div style={{fontSize:15,fontWeight:800,color:"#fff",letterSpacing:2.5}}>BHOOMISCAN</div><div style={{fontSize:9,color:"rgba(255,255,255,0.6)",letterSpacing:1.5}}>CONTENT ENGINE v3</div></div>
          </div>
      ...
      <div style={{maxWidth:960,margin:"0 auto",padding:"20px 16px 80px"}}>
        ...
        {vw==="home"&&<>
          ...
          {tH>0&&(()=>{const l=st.history[tH-1];return <div style={{...sC,borderLeft:`4px solid ${GL}`,background:"rgba(45,106,79,0.02)",boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
          ...
          <div style={{...sC,boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
            ...
            >
              ...
              <div style={{fontSize:11,color:TL,lineHeight:1.6}}>
                Supports: <span style={{fontWeight:600,color:BN}}>TXT, MD, CSV, JSON, PDF, DOC, DOCX</span>
              </div>
            ...
            {files.length>0&&<div style={{marginBottom:14}}>
              ...
              >
                <div style={{flex:1}}>
                  <div style={{fontSize:12,color:G,fontWeight:700,marginBottom:3}}>📄 {f.name}</div>
                  <div style={{fontSize:10,color:TL,display:"flex",alignItems:"center",gap:6}}>
                    <span style={{padding:"2px 8px",borderRadius:5,background:"rgba(45,106,79,0.15)",color:G,fontWeight:700,letterSpacing:0.3}}>{(f.type||'txt').toUpperCase()}</span>
          ...
          <div style={{...sC,borderLeft:`4px solid ${BN}`,background:"rgba(139,111,71,0.03)",boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
              ...
              <div>
                ...
                <div style={{fontSize:10,color:TL,marginTop:2}}>Run all 3 for best coverage. Each auto-includes today's date.</div>
              </div>
            </div>

            {getResearchPrompts().map((rp,i)=>{
        ...
        {/* LOG */}
        {vw==="log"&&(()=>{
          const parsed=parseLog(logT);
          const batchLabels=["Buyer","Seller","Agent+NRI","Trending"];
          return <>
          {/* Step 1: Paste Logs */}
          <div style={{...sC,borderLeft:`4px solid ${G}`,boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
            ...
            <div style={{background:"rgba(45,106,79,0.05)",padding:"12px 14px",borderRadius:8,marginBottom:12,borderLeft:`3px solid ${G}`}}>
              <div style={{fontSize:12,fontWeight:700,color:G,marginBottom:6}}>Paste all 4 variation log tables below</div>
              <div style={{fontSize:11,color:TL,lineHeight:1.6}}>
                Copy the variation log table from the bottom of each Claude batch output and paste them all together here.
              </div>
            </div>

            {/* Live Batch Detection Status */}
            <div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap"}}>
              {[0,1,2,3].map(i=>{
                const bd=parsed.details[i];
                const ok=bd&&bd.complete;
                const partial=bd&&!bd.complete;
                return <div key={i} style={{
                  flex:"1 1 auto",minWidth:100,padding:"8px 12px",borderRadius:8,textAlign:"center",
                  background:ok?"rgba(45,106,79,0.1)":partial?"rgba(180,64,64,0.08)":"rgba(139,111,71,0.04)",
                  border:`1.5px solid ${ok?G:partial?"#b44040":"#ddd5c8"}`,
                  transition:"all 0.3s ease"
                }}>
                  <div style={{fontSize:14,marginBottom:2}}>{ok?"✅":partial?"⚠️":"○"}</div>
                  <div style={{fontSize:10,fontWeight:700,color:ok?G:partial?"#b44040":TXL}}>Batch {i+1}</div>
                  <div style={{fontSize:9,color:TL}}>{batchLabels[i]}</div>
                  {bd&&<div style={{fontSize:9,color:ok?G:"#b44040",marginTop:2,fontWeight:600}}>
                    {ok?`${bd.count}/5`:partial?`${bd.count}/5 — missing ${bd.missing.join(", ")}`:""}</div>}
                </div>;
              })}
            </div>
            ...
            {logT.trim()&&<div style={{
              ...
              background:parsed.valid?"rgba(45,106,79,0.08)":parsed.scripts>0?"rgba(180,64,64,0.06)":"rgba(139,111,71,0.04)",
              border:`1.5px solid ${parsed.valid?G:parsed.scripts>0?"#b44040":BR}`
            }}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <div style={{fontSize:16}}>{parsed.valid?"✅":parsed.scripts>0?"⚠️":"📝"}</div>
                <div>
                  <div style={{fontSize:12,fontWeight:700,color:parsed.valid?G:parsed.scripts>0?"#b44040":BD}}>
                    {parsed.valid?`All ${parsed.completeBatches} Batches Complete — ${parsed.scripts} scripts detected`
                      :parsed.scripts>0?`${parsed.completeBatches}/4 batches complete (${parsed.scripts} scripts found)`
                      :"No scripts detected — check paste format"}
                  </div>
                  {!parsed.valid&&parsed.scripts>0&&<div style={{fontSize:10,color:TL,marginTop:2}}>
                    Paste the missing batch logs above to enable saving
                  </div>}
                </div>
              </div>
            </div>}
            ...
            <textarea
              ...
              placeholder={"Paste all 4 variation log tables here...\n\nExpected format (S1-S5 rows from each batch):\n| S1 | Story Entry | Conditional | Profile pe jaao... | SAVE | MEDIUM | Verified Props | Free Listings | Trust Deficit | Red flags buying | Talking Head |\n| S2 | Knowledge Gap | Process Integr. | Bhool mat jana... | SHARE | HIGH | Free Listings | WhatsApp Connect | Digital Illiteracy | Doc checklist | Myth Buster |\n| S3 | ... | ... | ... | ... | ... | ... | ... | ... | ... | ... |\n| S4 | ... | ... | ... | ... | ... | ... | ... | ... | ... | ... |\n| S5 | ... | ... | ... | ... | ... | ... | ... | ... | ... | ... |\n\n(Repeat for all 4 batches = 20 rows total)"}
            />
          ...
          {/* Step 3: Optional - Freshness Multipliers */}
          <div style={sC}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:8}}>
              {MULTIPLIERS.map(m=><button key={m.id} style={{padding:"5px 10px",borderRadius:6,fontSize:10,fontWeight:600,border:`1px solid ${BR}`,cursor:"pointer",
                background:Object.values(mults).includes(m.id)?"rgba(45,106,79,0.12)":"#faf8f4",
                color:Object.values(mults).includes(m.id)?G:TL}} title={m.desc}
          ...
          {(()=>{
            if(!cx){
              return <div style={{...sC,background:"rgba(180,64,64,0.04)",borderLeft:"4px solid #b44040"}}>
                <div style={{fontSize:13,color:"#b44040",fontWeight:600}}>Generate prompts first (Go to Generate tab)</div>
              </div>;
            }

            if(logT.trim()&&!parsed.valid){
              return <div style={{...sC,background:"rgba(180,64,64,0.06)",borderLeft:"4px solid #b44040",boxShadow:"0 2px 8px rgba(180,64,64,0.15)"}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                  ...
                  <div style={{fontSize:13,fontWeight:800,color:"#b44040"}}>Cannot Save — Need All 4 Batches</div>
                </div>
                <div style={{fontSize:11,color:"#b44040",lineHeight:1.6}}>
                  {parsed.completeBatches}/4 complete batches detected ({parsed.scripts} scripts).
                  {parsed.details.filter(d=>!d.complete).map((d,i)=>
                    <span key={i}> Batch {d.batch}: missing {d.missing.join(", ")}.</span>
                  )}
                </div>
                <div style={{fontSize:11,color:TL,marginTop:6,fontStyle:"italic"}}>
                  Each batch must have exactly S1-S5 rows. Copy the variation log from each prompt output.
                </div>
              </div>;
            }
            ...
            return <div style={{...sC,background:"rgba(45,106,79,0.04)",borderLeft:`4px solid ${G}`}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <div style={{fontSize:18}}>{parsed.valid?"✅":"📝"}</div>
                <div style={{fontSize:13,fontWeight:700,color:G}}>
                  {parsed.valid
                    ?`Ready to Save: Week ${cx.wk}, ${cx.mn} ${cx.yr} — Pattern ${rot?.pat} (${parsed.scripts} scripts)`
                    :`Ready to Save: Week ${cx.wk}, ${cx.mn} ${cx.yr} — Pattern ${rot?.pat} (no logs)`}
                </div>
              </div>
            </div>;
          })()}
          ...
          <div style={{display:"flex",gap:8,marginTop:10}}>
            <button
              style={{
                ...
                padding:14,
                fontSize:14,
                opacity:(!cx||(logT.trim()&&!parsed.valid))?0.5:1
              }}
              onClick={logWeek}
              disabled={!cx||(logT.trim()&&!parsed.valid)}
            >
              <span>✓</span>
