      ...
      <div style={{maxWidth:960,margin:"0 auto",padding:"20px 16px 80px"}}>
        ...
        {vw==="log"&&<>
          ...
          <div style={{...sC,borderLeft:`4px solid ${G}`,boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
            ...
            {logT.trim()&&(()=>{
              const scriptCount = (logT.match(/\|[^|]*Script[^|]*\|/gi)||[]).length - (logT.match(/\| Script \|/gi)||[]).length;
              const isComplete = scriptCount >= 20;
              const isPartial = scriptCount >= 5 && scriptCount < 20;
              const isEmpty = scriptCount === 0;

              return <div style={{marginBottom:10,padding:"10px 14px",borderRadius:8,background:isComplete?"rgba(45,106,79,0.08)":isPartial?"rgba(180,64,64,0.08)":"rgba(139,111,71,0.05)",border:`1.5px solid ${isComplete?G:isPartial?"#b44040":BR}`}}>
                <div style={{display:"flex",alignItems:"center",gap:8,justifyContent:"space-between",flexWrap:"wrap"}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <div style={{fontSize:20}}>{isComplete?"✅":isPartial?"⚠️":"📝"}</div>
                    <div>
                      <div style={{fontSize:12,fontWeight:700,color:isComplete?G:isPartial?"#b44040":BD}}>
                        {isComplete?"All 4 Batches Detected ✓":isPartial?`Only ${Math.floor(scriptCount/5)} Batch${scriptCount>5?"es":""} Detected!`:"Analyzing..."}
                      </div>
                      <div style={{fontSize:10,color:TL,marginTop:2}}>
                        {isEmpty?"Paste your variation logs below":`${scriptCount}/20 scripts found`}
                      </div>
                    </div>
                  </div>
          ...
          <div style={{display:"flex",gap:8,marginTop:10}}>
            <button
              style={{
                ...
                padding:14,
                fontSize:14,
                opacity:!cx || ((logT.match(/\|[^|]*Script[^|]*\|/gi)||[]).length - (logT.match(/\| Script \|/gi)||[]).length >= 5 && (logT.match(/\|[^|]*Script[^|]*\|/gi)||[]).length - (logT.match(/\| Script \|/gi)||[]).length < 20)?0.5:1
              }}
              onClick={logWeek}
              disabled={!cx || ((logT.match(/\|[^|]*Script[^|]*\|/gi)||[]).length - (logT.match(/\| Script \|/gi)||[]).length >= 5 && (logT.match(/\|[^|]*Script[^|]*\|/gi)||[]).length - (logT.match(/\| Script \|/gi)||[]).length < 20)}
            >
              <span>✓</span>
